...
void algoGrapheMat::purge(vector<SommetMat *> &sommets, int min_adjacents)
{
    ...
    while(sommets.front()->nbNeighboors() < min_adjacents){
...
GrapheMat* algoGrapheMat::BronKerboschWithGrowing(GrapheMat const* G)
{
    ...
    GrapheMat* cliqueTemp= maxCliqueGrowing(G, new GrapheMat("Clique de "+ G->getName()), consideredFirst);
    ...
    vector<SommetMat*>& consideredVertices = *(G->getAdjacents(sommetInitial)); //version avec les adjacents à un seul sommet
    consideredVertices.push_back(sommetInitial);
    // Suppression des sommets qui ont moins d'adjacents que la clique trouvée
    purge(consideredVertices, cliqueTemp->size());
    ...
    // Lancement algo
    GrapheMat* cliqueMax= maxCliqueBronKerboschV2_bis(G, new GrapheMat("Clique de "+ G->getName()), consideredVertices, *candidats);
